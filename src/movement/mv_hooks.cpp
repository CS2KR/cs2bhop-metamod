...
void FASTCALL movement::Detour_PhysicsSimulate(CCSPlayerController *controller)
{
	...
	g_BhopPlugin.simulatingPhysics = true;
	...
	g_BhopPlugin.simulatingPhysics = false;
}
											 ...
											 bool *bIsBhoping)
{
	...
	player->OnTryPlayerMove(pFirstDest, pFirstTrace, bIsBhoping);
	Vector oldVelocity = mv->m_vecVelocity;
	i32 count = traceHistory.Count();
	TryPlayerMove(ms, mv, pFirstDest, pFirstTrace, bIsBhoping);
	...
	player->OnTryPlayerMovePost(pFirstDest, pFirstTrace, bIsBhoping);
