...
void MovementPlayer::SetAngles(const QAngle &angles)
{
	...
	g_pBhopUtils->SnapViewAngles(pawn, angles);
}
...
void MovementPlayer::RegisterTakeoff(bool jumped, bool fromLadder, Vector *overrideOrigin)
{
	...
	this->takeoffTime = g_pBhopUtils->GetGlobals()->curtime - g_pBhopUtils->GetGlobals()->frametime;
...
void MovementPlayer::RegisterLanding(const Vector &landingVelocity, bool distbugFix)
{
	...
	this->landingTime = g_pBhopUtils->GetGlobals()->curtime;
	this->landingTimeServer = g_pBhopUtils->GetServerGlobals()->curtime;
		...
		FOR_EACH_VEC(mv->m_TouchList, i)
		{
			if (mv->m_TouchList[i].trace.m_vHitNormal.z > normal)
			{
				...
				this->landingTimeActual =
					this->landingTime
					- (1 - mv->m_TouchList[i].trace.m_flFraction) * g_pBhopUtils->GetGlobals()->frametime; // TODO: make sure this is right
				return;
			}
		}
	}
