...
bool utils::Initialize(ISmmAPI *ismm, char *error, size_t maxlen)
{
	...
	const char *gamedataPath = "addons/cs2bhop/gamedata/cs2bhop-core.games.txt";
	...
	g_pBhopUtils = new BhopUtils(GetLegacyGameEventListener, SnapViewAngles, EmitSound, SwitchTeam, SetPawn, CreateEntityByName, DispatchSpawn,
...
{
	if (strncmp(sound, "bhop.", strlen("bhop.")) == 0 && !g_BhopPlugin.IsAddonMounted())
	{
		return;
	}
	...
	g_pBhopUtils->EmitSound(filter, player.Get() + 1, soundParams);
}
...
void utils::ResetMapIfEmpty()
{
	...
	if (g_pBhopUtils->GetPlayerCount() > 0)
	{
		return;
	}

	// Don't restart if the server is just up to map reload loops.
	if (g_pBhopUtils->GetGlobals() && g_pBhopUtils->GetGlobals()->curtime < 30.0f)
	{
		return;
	}
	...
	META_CONPRINTF("[Bhop] Server is empty, triggering map reload...\n");
	utils::ResetMap();
}
...
void utils::ResetMap()
{
	...
	if (g_pBhopUtils->GetCurrentMapWorkshopID() == 0)
	{
		if (!g_pBhopUtils->GetGlobals() || !g_pBhopUtils->GetGlobals()->mapname.ToCStr() || g_pBhopUtils->GetGlobals()->mapname.ToCStr()[0] == 0)
		{
			META_CONPRINTF("[Bhop] Warning: Map name is empty, cannot reload the current map! Defaulting to de_dust2...\n");
			V_snprintf(cmd, sizeof(cmd), "changelevel de_dust2");
		}
		else
		{
			V_snprintf(cmd, sizeof(cmd), "changelevel %s", g_pBhopUtils->GetGlobals()->mapname.ToCStr());
		}
	}
	else
	{
		V_snprintf(cmd, sizeof(cmd), "host_workshop_map %llu", g_pBhopUtils->GetCurrentMapWorkshopID());
	}
...
bool utils::WriteBufferToFile(const char *relativePath, const std::vector<char> &buffer)
{
	...
	if (!fp)
	{
		META_CONPRINTF("[Bhop] Failed to open file for writing: %s\n", tmpPath);
		return false;
	}
...
bool utils::ReadBufferFromFile(const char *relativePath, std::vector<char> &outBuffer)
{
	...
	if (!fp)
	{
		META_CONPRINTF("[Bhop] Failed to open file for reading: %s\n", absPath);
		return false;
	}
...
void utils::RemoveFile(const char *relativePath)
{
	...
	if (remove(absPath) != 0)
	{
		META_CONPRINTF("[Bhop] Failed to remove %s\n", absPath);
	}
}
...
#endif
	{
		META_CONPRINTF("[Bhop] Failed to rename %s to %s\n", absOld, absNew);
		return false;
	}
	return true;
}
