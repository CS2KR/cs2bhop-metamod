					   ...
					   trace_t *pm)
{
	...
	for (u32 i = 0; i < 2; i++)
	{
		if (g_pBhopPlayerManager->ToPlayer(i) && g_pBhopPlayerManager->ToPlayer(i)->GetMoveServices())
		{
			error = g_pBhopPlayerManager->ToPlayer(i)->GetMoveServices()->m_flAccumulatedJumpError();
			velocity = g_pBhopPlayerManager->ToPlayer(i)->currentMoveData->m_vecVelocity;
			break;
		}
	}
...
void Detour_CPhysicsGameSystemFrameBoundary(void *pThis)
{
	...
	Bhop::misc::OnPhysicsGameSystemFrameBoundary(pThis);
}
