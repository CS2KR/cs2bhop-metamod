...
static_global void PrintCategoryCommands(BhopPlayer *player, i32 category, bool printEmpty)
{
	char tableName[64];
	V_snprintf(tableName, sizeof(tableName), "Command List - %s", cmdFlagNames[category]);
	CUtlString headers[BHOP_ARRAYSIZE(columnKeys)];
	for (u32 i = 0; i < BHOP_ARRAYSIZE(columnKeys); i++)
	{
		headers[i] = player->languageService->PrepareMessage(columnKeys[i]).c_str();
	}
	Scmd *cmds = g_cmdManager.cmds;
	utils::Table<BHOP_ARRAYSIZE(columnKeys)> table(player->languageService->PrepareMessage(tableName).c_str(), headers);
...
SCMD(bhop_help, SCFL_MISC)
{
	...
	if (args->ArgC() >= 2)
	{
		...
		{
			for (i32 j = 0; j < BHOP_ARRAYSIZE(cmdFlagNames); j++)
			{
	...
	if (!foundCategory)
	{
		...
		for (i32 i = 0; i < BHOP_ARRAYSIZE(cmdFlagNames); i++)
		{
...
META_RES scmd::OnClientCommand(CPlayerSlot &slot, const CCommand &args)
{
	...
	if (!controller || !g_pBhopPlayerManager->ToPlayer(controller))
	{
		return MRES_IGNORED;
	}
...
META_RES scmd::OnDispatchConCommand(ConCommandRef cmd, const CCommandContext &ctx, const CCommand &args)
{
	...
	if (!cmd.IsValidRef() || !controller || !g_pBhopPlayerManager->ToPlayer(controller))
	{
		return MRES_IGNORED;
	}
