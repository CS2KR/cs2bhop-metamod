...
#include "bhop/timer/queries/base_request.h"
...
static_function void AddEntityHooks(CBaseEntity *entity)
{
	...
	else if (V_strstr(entity->GetClassname(), "trigger_") || !V_stricmp(entity->GetClassname(), "player"))
	{
		...
		if (!V_stricmp(entity->GetClassname(), "player") && g_pBhopPlayerManager->ToPlayer(pawn))
		{
...
void EntListener::OnEntitySpawned(CEntityInstance *pEntity)
{
	if (V_strstr(pEntity->GetClassname(), "trigger_"))
	{
		...
		Bhop::mapapi::CheckEndTimerTrigger((CBaseTrigger *)pEntity);
	}
}
...
static_function void Hook_OnStartTouch(CBaseEntity *pOther)
{
	...
	if (BhopTriggerService::IsManagedByTriggerService(pThis, pOther) && !g_BhopPlugin.simulatingPhysics)
	{
...
static_function void Hook_OnTeleport(const Vector *newPosition, const QAngle *newAngles, const Vector *newVelocity)
{
	...
	if (this_->IsPawn())
	{
		MovementPlayer *player = g_pBhopPlayerManager->ToPlayer(static_cast<CBasePlayerPawn *>(this_));
...
static_function void Hook_OnChangeTeamPost(i32 team)
{
	...
	MovementPlayer *player = g_pBhopPlayerManager->ToPlayer(controller);
	if (player)
	{
...
static_function void Hook_CheckTransmit(CCheckTransmitInfo **pInfos, int infoCount, CBitVec<16384> &unk1, CBitVec<16384> &,
										const Entity2Networkable_t **pNetworkables, const uint16 *pEntityIndicies, int nEntities)
{
	Bhop::quiet::OnCheckTransmit(pInfos, infoCount);
	BhopProfileService::OnCheckTransmit();
	RETURN_META(MRES_IGNORED);
}
...
static_function void Hook_GameFrame(bool simulating, bool bFirstTick, bool bLastTick)
{
	...
	g_BhopPlugin.serverGlobals = *(g_pBhopUtils->GetGlobals());
	RunSubmission::CheckAll();
	BaseRequest::CheckRequests();
	BhopTelemetryService::ActiveCheck();
	BhopBeamService::UpdateBeams();
	BhopProfileService::OnGameFrame();
	Bhop::replaysystem::OnGameFrame();
	RETURN_META(MRES_IGNORED);
}
...
{
	g_steamAPI.Init();
	g_pBhopPlayerManager->OnSteamAPIActivated();
}
...
static_function bool Hook_ClientConnect(CPlayerSlot slot, const char *pszName, uint64 xuid, const char *pszNetworkID, bool unk1,
										CBufferString *pRejectReason)
{
	g_pBhopPlayerManager->OnClientConnect(slot, pszName, xuid, pszNetworkID, unk1, pRejectReason);
...
static_function void Hook_OnClientConnected(CPlayerSlot slot, const char *pszName, uint64 xuid, const char *pszNetworkID, const char *pszAddress,
											bool bFakePlayer)
{
	g_pBhopPlayerManager->OnClientConnected(slot, pszName, xuid, pszNetworkID, pszAddress, bFakePlayer);
	RETURN_META(MRES_IGNORED);
}

static_function void Hook_ClientFullyConnect(CPlayerSlot slot)
{
	g_pBhopPlayerManager->OnClientFullyConnect(slot);
	RETURN_META(MRES_IGNORED);
}

static_function void Hook_ClientPutInServer(CPlayerSlot slot, char const *pszName, int type, uint64 xuid)
{
	g_pBhopPlayerManager->OnClientPutInServer(slot, pszName, type, xuid);
	RETURN_META(MRES_IGNORED);
}

static_function void Hook_ClientActive(CPlayerSlot slot, bool bLoadGame, const char *pszName, uint64 xuid)
{
	g_pBhopPlayerManager->OnClientActive(slot, bLoadGame, pszName, xuid);
	BhopPlayer *player = g_pBhopPlayerManager->ToPlayer(slot);
	...
	{
		Warning("[Bhop] WARNING: Player pawn for slot %i not found!\n", slot.Get());
	}
	RETURN_META(MRES_IGNORED);
}
										   ...
										   const char *pszNetworkID)
{
	...
	g_pBhopPlayerManager->OnClientDisconnect(slot, reason, pszName, xuid, pszNetworkID);
	RETURN_META(MRES_IGNORED);
}

static_function void Hook_ClientVoice(CPlayerSlot slot)
{
	g_pBhopPlayerManager->OnClientVoice(slot);
}

static_function void Hook_ClientCommand(CPlayerSlot slot, const CCommand &args)
{
	VPROF_BUDGET(__func__, "CS2Bhop");
	if (META_RES result = Bhop::misc::CheckBlockedRadioCommands(args[0]))
	{
		RETURN_META(result);
	}
...
static_function void Hook_StartupServer(const GameSessionConfiguration_t &config, ISource2WorldSession *, const char *)
{
	g_BhopPlugin.AddonInit();
	Bhop::course::ClearCourses();
	Bhop::mapapi::Init();
...
static_function bool Hook_FireEvent(IGameEvent *event, bool bDontBroadcast)
{
	if (event)
	{
		if (BHOP_STREQI(event->GetName(), "player_death"))
		{
			CEntityInstance *instance = event->GetPlayerPawn("userid");
			BhopPlayer *player = g_pBhopPlayerManager->ToPlayer(instance->GetEntityIndex());
			if (player)
			{
		...
		else if (BHOP_STREQI(event->GetName(), "round_prestart"))
		{
			hooks::HookEntities();
			Bhop::mapapi::OnRoundPreStart();
		}
		else if (BHOP_STREQI(event->GetName(), "round_start"))
		{
			interfaces::pEngine->ServerCommand("sv_full_alltalk 1");
			BhopTimerService::OnRoundStart();
			Bhop::misc::OnRoundStart();
			Bhop::mapapi::OnRoundStart();
			Bhop::replaysystem::OnRoundStart();
		}
		else if (BHOP_STREQI(event->GetName(), "player_team"))
		{
			event->SetBool("silent", true);
		}
		else if (BHOP_STREQI(event->GetName(), "player_spawn"))
		{
			CEntityInstance *instance = event->GetPlayerPawn("userid");
			if (instance)
			{
...
static_function void Hook_DispatchConCommand(ConCommandRef cmd, const CCommandContext &ctx, const CCommand &args)
{
	...
	if (BhopOptionService::GetOptionInt("overridePlayerChat", true))
	{
		Bhop::misc::ProcessConCommand(cmd, ctx, args);
	}
...
static_function void Hook_PostEvent(CSplitScreenSlot nSlot, bool bLocalOnly, int nClientCount, const uint64 *clients, INetworkMessageInternal *pEvent,
									const CNetMessage *pData, unsigned long nSize, NetChannelBufType_t bufType)
{
	Bhop::quiet::OnPostEvent(pEvent, pData, clients);
}

// CEntitySystem
static_function void Hook_CEntitySystem_Spawn(int nCount, const EntitySpawnInfo_t *pInfo)
{
	Bhop::mapapi::OnSpawn(nCount, pInfo);
}
...
static_function bool Hook_ActivateServer()
{
	...
	{
		BhopPlayer *player = g_pBhopPlayerManager->ToPlayer(CPlayerSlot(0));
		player->Reset();
	}
	u64 id = g_pBhopUtils->GetCurrentMapWorkshopID();
	u64 size = g_pBhopUtils->GetCurrentMapSize();

	META_CONPRINTF("[Bhop] Loading map %s, workshop ID %llu, size %llu\n", g_pBhopUtils->GetCurrentMapVPK().Get(), id, size);

	RunSubmission::Clear();
	Bhop::misc::OnActivateServer();
	BhopDatabaseService::SetupMap();
	BhopGlobalService::OnActivateServer();
	BhopRecordingService::OnActivateServer();

	char md5[33];
	g_pBhopUtils->GetCurrentMapMD5(md5, sizeof(md5));
	META_CONPRINTF("[Bhop] Map file md5: %s\n", md5);
...
{
	g_pBhopPlayerManager->OnConnectClient(pszName, pAddr, steam_handle, pConnectMsg, pszChallenge, pAuthTicket, nAuthTicketLength, bIsLowViolence);
...
static_function CServerSideClientBase *Hook_ConnectClientPost(const char *pszName, ns_address *pAddr, uint32 steam_handle,
															  ...
															  int nAuthTicketLength, bool bIsLowViolence)
{
	g_pBhopPlayerManager->OnConnectClientPost(pszName, pAddr, steam_handle, pConnectMsg, pszChallenge, pAuthTicket, nAuthTicketLength,
											  bIsLowViolence);
	RETURN_META_VALUE(MRES_IGNORED, 0);
}
...
{
	ProcessTimers();
	BhopRecordingService::ProcessFileWriteCompletion();
	if (g_asyncFileIO)
	{
		g_asyncFileIO->RunFrame();
	}
	BhopGlobalService::OnServerGamePostSimulate();
}

static_function void Hook_BuildGameSessionManifest(const EventBuildGameSessionManifest_t *msg)
{
	Warning("[CS2Bhop] IGameSystem::BuildGameSessionManifest\n");
	IEntityResourceManifest *pResourceManifest = msg->m_pResourceManifest;
	if (g_BhopPlugin.IsAddonMounted())
	{
		Warning("[CS2Bhop] Precache bhop soundevents \n");
		pResourceManifest->AddResource(BHOP_WORKSHOP_ADDON_SNDEVENT_FILE);
	}
...
static_function ILoadingSpawnGroup *Hook_OnCreateLoadingSpawnGroupHook(SpawnGroupHandle_t hSpawnGroup, bool bSynchronouslySpawnEntities,
																	   ...
																	   const CUtlVector<const CEntityKeyValues *> *pKeyValues)
{
	Bhop::mapapi::OnCreateLoadingSpawnGroupHook(pKeyValues);
	RETURN_META_VALUE(MRES_IGNORED, 0);
}
...
static_function void Hook_OnPlayerRunCommand(PlayerCommand *pCmd)
{
	...
	BhopPlayer *player = g_pBhopPlayerManager->ToPlayer(pThis);
	Bhop::replaysystem::OnPlayerRunCommandPre(player, pCmd);
	RETURN_META(MRES_IGNORED);
}
...
static_function void Hook_OnFinishMove(PlayerCommand *pCmd, CMoveData *pMoveData)
{
	CCSPlayer_MovementServices *pThis = META_IFACEPTR(CCSPlayer_MovementServices);
	BhopPlayer *player = g_pBhopPlayerManager->ToPlayer(pThis);
	Bhop::replaysystem::OnFinishMovePre(player, pMoveData);
	RETURN_META(MRES_IGNORED);
}
