...
static_global class BhopOptionServiceEventListener_Beam : public BhopOptionServiceEventListener
{
...
void BhopBeamService::OnPlayerPreferencesLoaded()
{
	this->SetBeamType(this->player->optionService->GetPreferenceInt("desiredBeamType"));
	this->playerBeamOffset = this->player->optionService->GetPreferenceVector("beamOffset", BhopBeamService::defaultOffset);
}

void BhopBeamService::Init()
{
	BhopOptionService::RegisterEventListener(&optionEventListener);
}

SCMD(bhop_beam, SCFL_MISC | SCFL_PREFERENCE)
{
	BhopPlayer *player = g_pBhopPlayerManager->ToPlayer(controller);
	u8 newDesiredBeamType = (player->beamService->desiredBeamType + 1) % BhopBeamService::BEAM_COUNT;
	if (args->ArgC() >= 2)
	{
		if (BHOP_STREQI(args->Arg(1), "off"))
		{
			newDesiredBeamType = BhopBeamService::BEAM_NONE;
		}
		else if (BHOP_STREQI(args->Arg(1), "on"))
		{
			newDesiredBeamType = BhopBeamService::BEAM_FEET;
		}
		else
		{
...
SCMD_LINK(bhop_trail, bhop_beam);

SCMD(bhop_beamoffset, SCFL_MISC | SCFL_PREFERENCE)
{
	BhopPlayer *player = g_pBhopPlayerManager->ToPlayer(controller);
...
void BhopBeamService::Update()
{
	BhopPlayer *newTarget = this->player->IsAlive() ? this->player : this->player->specService->GetSpectatedPlayer();

	if (this->target != newTarget)
	{
		if (this->playerBeam.Get())
		{
			g_pBhopUtils->RemoveEntity(this->playerBeam.Get());
		}
		if (this->playerBeamNew.Get())
		{
			g_pBhopUtils->RemoveEntity(this->playerBeamNew.Get());
		}
		this->playerBeam = {};
		this->playerBeamNew = {};
...
void BhopBeamService::UpdatePlayerBeam()
{
	bool shouldDraw = this->desiredBeamType != BhopBeamService::BEAM_NONE;
	...
	if (beam && g_pBhopUtils->GetServerGlobals()->curtime - beam->m_flStartTime().GetTime() > 3.0f)
	{
		if (!this->playerBeamNew.Get())
		{
		...
		else if (g_pBhopUtils->GetServerGlobals()->curtime - beam->m_flStartTime().GetTime() > 3.2f)
		{
			g_pBhopUtils->RemoveEntity(beam);
...
void BhopBeamService::Reset()
{
	this->playerBeam = {};
	this->playerBeamNew = {};
	this->playerBeamOffset = BhopBeamService::defaultOffset;
	this->target = {};
	this->validBeam = {};
...
void BhopBeamService::UpdateBeams()
{
