...
BaseRequest::BaseRequest(u64 uid, BhopPlayer *player)
	: uid(uid), userID(player->GetClient()->GetUserID()), timestamp(g_pBhopUtils->GetServerGlobals()->realtime)
{
}
...
void BaseRequest::Init(u64 features, const CCommand *args, bool queryLocal, bool queryGlobal)
{
	...
	KeyValues3 params;
	if (!utils::ParseArgsToKV3(args->ArgS(), params, const_cast<const char **>(paramKeys), BHOP_ARRAYSIZE(paramKeys)))
	{
		this->Invalidate("");
		this->PrintInstructions();
		return;
	}
	this->localStatus = (queryLocal && BhopDatabaseService::IsReady()) ? ResponseStatus::ENABLED : ResponseStatus::DISABLED;

	this->globalStatus = (queryGlobal && BhopGlobalService::IsAvailable() ? ResponseStatus::ENABLED : ResponseStatus::DISABLED);
	...
	if (this->HasFeature(RequestFeature::Map))
	{
		...
		if (this->mapName.IsEmpty())
		{
			...
			CUtlString currentMap = g_pBhopUtils->GetCurrentMapName(&gotCurrentMap);
...
void BaseRequest::CheckRequests()
{
	...
	instances.erase(std::remove_if(instances.begin(), instances.end(),
		[](BaseRequest *instance)
		{
			...
			else if (instance->timestamp + instance->timeout < g_pBhopUtils->GetServerGlobals()->realtime)
			{
...
void BaseRequest::SetupCourse(CUtlString courseName)
{
	...
	BhopPlayer *callingPlayer = g_pBhopPlayerManager->ToPlayer(userID);
	...
	if (this->mapName == currentMap)
	{
		...
		const BhopCourseDescriptor *course = callingPlayer->timerService->GetCourse();
		if (!course)
		{
	...
	else if (this->localStatus == ResponseStatus::ENABLED)
	{
		...
		BhopDatabaseService::FindFirstCourseByMapName(mapName, onQuerySuccess, onQueryFailure);
	}
	else
	{
		courseName = "1";
...
void BaseRequest::SetupPlayer(CUtlString playerName)
{
	...
	BhopPlayer *callingPlayer = g_pBhopPlayerManager->ToPlayer(this->userID);
	if (!callingPlayer)
	{
		this->Invalidate();
	}
	...
	if (this->localStatus == ResponseStatus::ENABLED)
	{
		...
		BhopDatabaseService::FindPlayerByAlias(playerName, onQuerySuccess, onQueryFailure);
	}
...
void BaseRequest::SetupMode(CUtlString modeName)
{
	...
	if (modeName.IsEmpty())
	{
		BhopModeManager::ModePluginInfo modeInfo = Bhop::mode::GetModeInfo(callingPlayer->modeService);
	...
	else
	{
		...
		BhopModeManager::ModePluginInfo modeInfo = Bhop::mode::GetModeInfo(modeName);
	...
	if (!Bhop::API::DecodeModeString(this->modeName.Get(), this->apiMode))
	{
...
void BaseRequest::SetupStyles(CUtlString styleNames)
{
	...
	if (styleNames.IsEmpty())
	{
		...
		{
			BhopStyleManager::StylePluginInfo info = Bhop::style::GetStyleInfo(callingPlayer->styleServices[i]);
	...
	else
	{
		...
		{
			BhopStyleManager::StylePluginInfo info = Bhop::style::GetStyleInfo(stylesSplit[i]);
			if (info.databaseID < 0)
			{
				this->localStatus = ResponseStatus::DISABLED;
			}
