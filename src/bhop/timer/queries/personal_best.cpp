...
struct PBRequest : public BaseRequest
{
	...
	virtual void QueryGlobal()
	{
		...
		auto callback = [uid = this->uid](Bhop::API::events::PersonalBest &pb)
		{
		...
		BhopGlobalService::QueryPB(this->targetSteamID64, std::string_view(this->targetPlayerName.Get(), this->targetPlayerName.Length()),
	...
	void ReplyGlobal()
	{
		...
		if (!globallyBanned)
		{
			...
			{
				// Bhop | Global: 12.34 [Overall / 10000 pts]
	...
	void ExecuteStandardLocalQuery()
	{
		...
		BhopDatabaseService::QueryPB(targetSteamID64, this->mapName, this->courseName, this->localModeID, onQuerySuccess, onQueryFailure);
	}
	...
	void ExecuteRanklessLocalQuery()
	{
		...
		BhopDatabaseService::QueryPBRankless(targetSteamID64, mapName, courseName, localModeID, localStyleIDs, onQuerySuccess, onQueryFailure);
	}
};

SCMD(bhop_pb, SCFL_RECORD)
{
...
/* TODO: global
SCMD(bhop_gpb, SCFL_RECORD | SCFL_GLOBAL)
{
	BhopPlayer *player = g_pBhopPlayerManager->ToPlayer(controller);
