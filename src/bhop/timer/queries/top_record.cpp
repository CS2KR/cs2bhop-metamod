...
struct TopRecordRequest : public BaseRequest
{
	...
	virtual void QueryGlobal()
	{
		...
		{
			auto callback = [uid = this->uid](Bhop::API::events::WorldRecords &wrs)
			{
			...
			BhopGlobalService::QueryWorldRecords(std::string_view(this->mapName.Get(), this->mapName.Length()),
	...
	virtual void QueryLocal()
	{
		...
		if (this->localStatus == ResponseStatus::ENABLED)
		{
			...
			BhopDatabaseService::QueryRecords(this->mapName, this->courseName, this->localModeID, 1, 0, onQuerySuccess, onQueryFailure);
		}
	}
	...
	void ReplyGlobal()
	{
		...
		{
			// Bhop | Overall Record: 01:23.45 by Bill
...
/* TODO: global
SCMD(bhop_wr, SCFL_RECORD | SCFL_GLOBAL)
{
