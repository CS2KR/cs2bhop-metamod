...
class BhopGlobalService : public BhopBaseService
{
...
public:
	 ...
	 *
	 * `F` should be a function that accepts a single argument of type `const Bhop::API::Map*`.
	 */
	template<typename F>
	static auto WithCurrentMap(F &&f)
	{
		std::unique_lock lock(BhopGlobalService::currentMap.mutex);
		const Bhop::API::Map *currentMap = nullptr;

		if (BhopGlobalService::currentMap.data.has_value())
		{
			currentMap = &*BhopGlobalService::currentMap.data;
		}
	 ...
	 * `const std::vector<Bhop::API::handshake::HelloAck::ModeInfo>&`.
	 */
	template<typename F>
	static auto WithGlobalModes(F &&f)
	{
		std::unique_lock lock(BhopGlobalService::globalModes.mutex);
		const std::vector<Bhop::API::handshake::HelloAck::ModeInfo> &modes = BhopGlobalService::globalModes.data;
		return f(modes);
	}
	 ...
	 *
	 * `F` should be a function that accepts a single argument of type
	 * `const std::vector<Bhop::API::handshake::HelloAck::StyleInfo>&`.
	 */
	template<typename F>
	static auto WithGlobalStyles(F &&f)
	{
		std::unique_lock lock(BhopGlobalService::globalStyles.mutex);
		const std::vector<Bhop::API::handshake::HelloAck::StyleInfo> &styles = BhopGlobalService::globalStyles.data;
		return f(styles);
	}
...
public:
	...
	SubmitRecordResult SubmitRecord(u16 filterID, f64 time, std::string_view modeMD5, void *styles, std::string_view metadata, CB &&cb)
	{
		...
		{
			std::unique_lock currentMapLock(BhopGlobalService::currentMap.mutex);
			if (!BhopGlobalService::currentMap.data.has_value())
			{
				META_CONPRINTF("[Bhop::Global] Cannot submit record on non-global map.\n");
				return SubmitRecordResult::MapNotGlobal;
			}
		}

		Bhop::API::events::NewRecord data;
		...
		switch (BhopGlobalService::state.load())
		{
			case BhopGlobalService::State::HandshakeCompleted:
				BhopGlobalService::SendMessage("new-record", data, cb);
			...
			default:
				BhopGlobalService::AddMainThreadCallback([=]() { BhopGlobalService::SendMessage("new-record", data, cb); });
	...
	static bool QueryPB(u64 steamid64, std::string_view targetPlayerName, std::string_view mapName, std::string_view courseNameOrNumber,
						Bhop::API::Mode mode, const CUtlVector<CUtlString> &styleNames, CB &&cb)
	{
		...
		Bhop::API::events::WantPersonalBest data = {steamid64, targetPlayerName, mapName, courseNameOrNumber, mode};
		FOR_EACH_VEC(styleNames, i)
		{
			data.styles.emplace_back(styleNames[i].Get());
		}
		return BhopGlobalService::SendMessage(event, data, std::move(cb));
	}
	 ...
	 */
	template<typename CB>
	static bool QueryCourseTop(std::string_view mapName, std::string_view courseNameOrNumber, Bhop::API::Mode mode, u32 limit, u32 offset, CB &&cb)
	{
		if (!BhopGlobalService::IsAvailable())
		{
			return false;
		...
		Bhop::API::events::WantCourseTop data = {mapName, courseNameOrNumber, mode, limit, offset};
		return BhopGlobalService::SendMessage(event, data, std::move(cb));
	}
	 ...
	 */
	template<typename CB>
	static bool QueryWorldRecords(std::string_view mapName, std::string_view courseNameOrNumber, Bhop::API::Mode mode, CB &&cb)
	{
		if (!BhopGlobalService::IsAvailable())
		{
			return false;
		...
		Bhop::API::events::WantWorldRecords data = {mapName, courseNameOrNumber, mode};
		return BhopGlobalService::SendMessage(event, data, std::move(cb));
	}
...
private:
	...
	{
		std::mutex mutex;
		std::optional<Bhop::API::Map> data;
	} currentMap {};
	...
	{
		std::mutex mutex;
		std::vector<Bhop::API::handshake::HelloAck::ModeInfo> data;
	} globalModes;
	...
	{
		std::mutex mutex;
		std::vector<Bhop::API::handshake::HelloAck::StyleInfo> data;
	} globalStyles;
	 ...
	 * Has to be called from the main thread.
	 */
	static void CompleteHandshake(Bhop::API::handshake::HelloAck &ack);
	...
	{
		std::unique_lock lock(BhopGlobalService::mainThreadCallbacks.mutex);
		BhopGlobalService::mainThreadCallbacks.queue.emplace_back(std::move(callback));
	}
	...
	{
		std::unique_lock lock(BhopGlobalService::mainThreadCallbacks.mutex);
		BhopGlobalService::mainThreadCallbacks.whenConnectedQueue.emplace_back(std::move(callback));
	}
	...
	static void AddMessageCallback(u32 messageID, CB &&callback)
	{
		...
		BhopGlobalService::messageCallbacks.queue[messageID] = std::move(callback);
	}
	 ...
	 */
	static bool PrepareMessage(std::string_view event, u32 messageID, const Bhop::API::handshake::Hello &data, Json &payload)
	{
		if (BhopGlobalService::state.load() != State::Connected)
		{
			META_CONPRINTF("[Bhop::Global] WARN: called `SendMessage()` before connection was established (state=%i)\n",
						   BhopGlobalService::state.load());
			return false;
		}
	...
	{
		if (BhopGlobalService::state.load() != State::HandshakeCompleted)
		{
			META_CONPRINTF("[Bhop::Global] WARN: called `SendMessage()` before handshake has completed (state=%i)\n",
						   BhopGlobalService::state.load());
			return false;
		}
		...
		if (!success)
		{
			META_CONPRINTF("[Bhop::Global] Failed to serialize message for event `%s`.\n", event);
		}
	...
	static bool SendMessage(std::string_view event, const T &data)
	{
		...
		if (!BhopGlobalService::PrepareMessage(event, BhopGlobalService::nextMessageID++, data, payload))
		{
			return false;
		}

		BhopGlobalService::socket->send(payload.ToString());
		return true;
	}
	...
	{
		u32 messageID = BhopGlobalService::nextMessageID++;
		Json payload;

		if (!BhopGlobalService::PrepareMessage(event, messageID, data, payload))
		{
			return false;
		}

		// clang-format off
		BhopGlobalService::AddMessageCallback(messageID, [callback = std::move(callback)](u32 messageID, const Json& payload)
		{
			if (!payload.IsValid())
			{
			...
			{
				META_CONPRINTF("[Bhop::Global] WebSocket message does not contain a valid `data` field.\n");
				return;
			}
