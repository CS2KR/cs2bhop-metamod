...
static_global class BhopOptionServiceEventListener_Quiet : public BhopOptionServiceEventListener
{
...
void Bhop::quiet::OnCheckTransmit(CCheckTransmitInfo **pInfo, int infoCount)
{
	for (int i = 0; i < infoCount; i++)
	{
		...
		BhopPlayer *targetPlayer = g_pBhopPlayerManager->ToPlayer(targetSlot);
		...
		// clang-format on
		{
			...
			u32 index = g_pBhopPlayerManager->ToPlayer(pawn)->index;
...
static void FilterQuietClients(const uint64 *clients, u32 emitterPlayerIndex = 0)
{
	...
	{
		BhopPlayer *recipient = g_pBhopPlayerManager->ToPlayer(recipientPlayerIndex);
...
void Bhop::quiet::OnPostEvent(INetworkMessageInternal *pEvent, const CNetMessage *pData, const uint64 *clients)
{
	...
	switch (info->m_MessageId)
	{
		...
		// Used by bhop_misc to block valve's player say messages.
		case CS_UM_SayText:
		case UM_SayText:
		{
	...
	if (emitterEnt->IsPawn())
	{
		...
		u32 emitterPlayerIndex = g_pBhopPlayerManager->ToPlayer(utils::GetController(pawn))->index;
	...
	else if (V_strstr(emitterEnt->GetClassname(), "weapon_"))
	{
		...
		if (emitterEnt->m_hOwnerEntity().IsValid() && emitterEnt->m_hOwnerEntity.Get()->IsPawn())
		{
			...
			u32 emitterPlayerIndex = g_pBhopPlayerManager->ToPlayer(ownerPawn)->index;
			FilterQuietClients(clients, emitterPlayerIndex);
		}
...
void BhopQuietService::SendFullUpdate()
{
	if (CServerSideClient *client = g_pBhopUtils->GetClientBySlot(this->player->GetPlayerSlot()))
	{
...
bool BhopQuietService::ShouldHide()
{
...
bool BhopQuietService::ShouldHideIndex(u32 targetIndex)
{
	// Don't self-hide.
...
SCMD(bhop_hideweapon, SCFL_PLAYER)
{
...
SCMD_LINK(bhop_hw, bhop_hideweapon);

void BhopQuietService::ToggleHideWeapon()
{
...
void BhopQuietService::OnPhysicsSimulatePost() {}

void BhopQuietService::OnPlayerPreferencesLoaded()
{
	this->hideWeapon = this->player->optionService->GetPreferenceBool("hideWeapon", false);
	if (this->hideWeapon)
	{
...
void BhopQuietService::ToggleHide()
{
...
void BhopQuietService::UpdateHideState()
{
