...
CConVar<bool> bhop_profile_rating_badge_enabled("bhop_profile_rating_badge_enabled", FCVAR_NONE, "Whether to show competitive rank in scoreboard.",
												true);

void BhopProfileService::OnGameFrame()
{
	if (g_pBhopUtils->GetServerGlobals()->tickcount % 64 != 0)
	{
		return;
	}
...
void BhopProfileService::OnCheckTransmit()
{
...
void BhopProfileService::RequestRating()
{
	if (!BhopGlobalService::IsAvailable() || !this->player->IsAuthenticated() || !this->player->IsConnected())
	{
		return;
	}
	Bhop::API::Mode mode;
	if (!Bhop::API::DecodeModeString(this->player->modeService->GetModeShortName(), mode))
	{
		return;
	}
	...
	this->timeToNextRatingRefresh = g_pBhopUtils->GetServerGlobals()->realtime + RATING_REFRESH_PERIOD + RandomFloat(-30.0f, 30.0f);
	std::string url = std::string(BhopOptionService::GetOptionStr("apiUrl", "https://")) + "/players/" + std::to_string(steamID64)
	...
	auto callback = [steamID64, mode](HTTP::Response response)
	{
		...
		BhopPlayer *player = g_pBhopPlayerManager->SteamIdToPlayer(steamID64);
		if (player == nullptr)
		{
			return;
		}
...
bool BhopProfileService::CanDisplayRank()
{
	...
	Bhop::API::Mode mode;
	return Bhop::API::DecodeModeString(this->player->modeService->GetModeShortName(), mode);
}

void BhopProfileService::UpdateClantag()
{
...
void BhopProfileService::OnPhysicsSimulatePost()
{
	if (g_pBhopUtils->GetServerGlobals()->realtime >= this->timeToNextRatingRefresh)
	{
		this->RequestRating();
	}
}

void BhopProfileService::UpdateCompetitiveRank()
{
	if (!this->player->GetController() || !bhop_profile_rating_badge_enabled.GetBool())
	{
		return;
	}
...
std::string BhopProfileService::GetPrefix(bool colors)
{
	if (this->clanTag[0] == '\0')
	{
		this->UpdateClantag();
