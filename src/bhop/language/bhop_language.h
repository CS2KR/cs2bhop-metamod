...
class BhopLanguageService : public BhopBaseService
{
...
public:
	...
	std::string PrepareMessage(const char *message, Args &&...args)
	{
		...
		return BhopLanguageService::PrepareMessageWithLang(language, message, args...);
	}
...
private:
	...
	template<typename... Args>
	static void PrintType(BhopPlayer *player, bool addPrefix, MessageType type, const char *message, Args &&...args)
	{
	...
	template<typename... Args>
	static void PrintSingle(BhopPlayer *player, bool addPrefix, bool includeSpectators, MessageType type, const char *message, Args &&...args)
	{
...
#define REGISTER_PRINT_ALL_FUNCTION(name, type) \
	...
	{ \
		...
		{ \
			...
			if (controller) \
			{ \
				PrintSingle(g_pBhopPlayerManager->ToPlayer(i), addPrefix, false, type, message, args...); \
			} \
		} \
	}
