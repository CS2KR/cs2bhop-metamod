...
ModeServiceFactory g_ModeFactory = [](BhopPlayer *player) -> BhopModeService * { return new BhopSourceModeService(player); };
PLUGIN_EXPOSE(BhopSourceModePlugin, g_BhopSourceModePlugin);

bool BhopSourceModePlugin::Load(PluginId id, ISmmAPI *ismm, char *error, size_t maxlen, bool late)
{
	PLUGIN_SAVEVARS();
	// Load mode
	int success;
...
bool BhopSourceModePlugin::Unload(char *error, size_t maxlen)
{
	g_pModeManager->UnregisterMode(g_PLID);
	return true;
}

bool BhopSourceModePlugin::Pause(char *error, size_t maxlen)
{
	g_pModeManager->UnregisterMode(g_PLID);
	return true;
}

bool BhopSourceModePlugin::Unpause(char *error, size_t maxlen)
{
	if (!g_pModeManager->RegisterMode(g_PLID, MODE_NAME_SHORT, MODE_NAME, g_ModeFactory))
	{
		return false;
...
void BhopSourceModeService::Reset()
{
...
void BhopSourceModeService::Cleanup()
{
...
const char *BhopSourceModeService::GetModeName()
{
	return MODE_NAME;
}

const char *BhopSourceModeService::GetModeShortName()
{
	return MODE_NAME_SHORT;
}

const CVValue_t *BhopSourceModeService::GetModeConVarValues()
{
	return modeCvarValues;
}

void BhopSourceModeService::OnSetupMove(PlayerCommand *pc)
{
...
void BhopSourceModeService::OnPhysicsSimulate()
{
...
void BhopSourceModeService::OnPhysicsSimulatePost()
{
	CCSPlayer_MovementServices *moveServices = this->player->GetMoveServices();
	if (!moveServices)
	{
