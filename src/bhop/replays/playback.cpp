...
namespace Bhop::replaysystem::playback
{
	...
	void OnPhysicsSimulate(BhopPlayer *player)
	{
		...
		moveServices->m_LegacyJump().m_flJumpPressedTime =
			g_pBhopUtils->GetServerGlobals()->curtime + tickData->pre.jumpPressedTime - tickData->gameTime;

		moveServices->m_flDuckSpeed = tickData->pre.duckSpeed;
		moveServices->m_flDuckAmount = tickData->pre.duckAmount;
		moveServices->m_flLastDuckTime = g_pBhopUtils->GetServerGlobals()->curtime + tickData->pre.lastDuckTime - tickData->gameTime;
	...
	void OnPhysicsSimulatePost(BhopPlayer *player)
	{
		...
		moveServices->m_LegacyJump().m_flJumpPressedTime =
			g_pBhopUtils->GetServerGlobals()->curtime + tickData->post.jumpPressedTime - tickData->gameTime;

		moveServices->m_flDuckSpeed = tickData->post.duckSpeed;
		moveServices->m_flDuckAmount = tickData->post.duckAmount;
		moveServices->m_flLastDuckTime = g_pBhopUtils->GetServerGlobals()->curtime + tickData->post.lastDuckTime - tickData->gameTime;
	...
	void CheckWeapon(BhopPlayer &player, PlayerCommand &cmd)
	{
		...
		if (desiredWeapon != activeWeapon)
		{
			...
			gear_slot_t desiredGearSlot = Bhop::replaysystem::item::GetWeaponGearSlot(desiredWeapon.mainInfo.itemDef);
			for (i32 i = 0; i < weapons->Count(); i++)
			{
				CBasePlayerWeapon *invWeapon = weapons->Element(i).Get();
				gear_slot_t invGearSlot =
					Bhop::replaysystem::item::GetWeaponGearSlot(invWeapon->m_AttributeManager().m_Item().m_iItemDefinitionIndex());
			...
			std::string weaponName = Bhop::replaysystem::item::GetWeaponName(desiredWeapon.mainInfo.itemDef);
			CBasePlayerWeapon *newWeapon = player.GetPlayerPawn()->m_pItemServices()->GiveNamedItem(weaponName.c_str());
			if (newWeapon)
			{
				Bhop::replaysystem::item::ApplyItemAttributesToWeapon(*newWeapon, desiredWeapon);
