...
namespace Bhop::replaysystem::commands
{
	...
	void NavigateReplay(BhopPlayer *player, u32 targetTick)
	{
		...
		if (bot)
		{
			BhopPlayer *botPlayer = g_pBhopPlayerManager->ToPlayer(bot);
			if (botPlayer)
			{
	...
	void LoadReplay(BhopPlayer *player, const char *uuid)
	{
		...
		char replayPath[512];
		V_snprintf(replayPath, sizeof(replayPath), BHOP_REPLAY_PATH "/%s.replay", parsedUuid.ToString().c_str());
		...
		data::LoadReplayAsync(
			...
			data::LoadSuccessCallback([playerUserID]() {
				BhopPlayer* player = g_pBhopPlayerManager->ToPlayer(playerUserID);
				if (!player)
				{
					return;
				}
				if (!BHOP_STREQI(data::GetCurrentReplay()->header.map().name().c_str(), g_pBhopUtils->GetCurrentMapName().Get()))
				{
					player->languageService->PrintChat(true, false, "Replay - Wrong Map", 
						data::GetCurrentReplay()->header.map().name().c_str(), g_pBhopUtils->GetCurrentMapName().Get());
					return;
				}
				for (u32 i = 0; i < data::GetCurrentReplay()->numEvents; i++)
				{
					...
					switch (event.type)
					{
						...
						{
							if (Bhop::mode::GetModeInfo(CUtlString(event.data.modeChange.name)).id < 0)
							{
						...
						{
							if (event.data.styleChange.name[0] != '\0' && Bhop::style::GetStyleInfo(CUtlString(event.data.styleChange.name)).id < 0)
							{
	...
	void ListReplays(BhopPlayer *player, const char *filter)
	{
		if (!player)
		{
			return;
...
SCMD(bhop_replay, SCFL_REPLAY)
{
	...
	Bhop::replaysystem::commands::LoadReplay(player, args->Arg(1));
	return MRES_SUPERCEDE;
}

SCMD(bhop_rpgoto, SCFL_REPLAY)
{
	BhopPlayer *player = g_pBhopPlayerManager->ToPlayer(controller);
	if (!player)
	{
	...
	Bhop::replaysystem::commands::JumpToReplayTime(player, args->ArgS());
	return MRES_SUPERCEDE;
}

SCMD(bhop_rpgototick, SCFL_REPLAY)
{
	BhopPlayer *player = g_pBhopPlayerManager->ToPlayer(controller);
	if (!player)
	{
	...
	Bhop::replaysystem::commands::JumpToReplayTick(player, args->Arg(1));
	return MRES_SUPERCEDE;
}

SCMD(bhop_rpinfo, SCFL_REPLAY)
{
	BhopPlayer *player = g_pBhopPlayerManager->ToPlayer(controller);
	if (!player)
	{
	...
	Bhop::replaysystem::commands::GetReplayInfo(player);
	return MRES_SUPERCEDE;
}

SCMD_LINK(bhop_rpseek, bhop_rpgoto);

SCMD(bhop_rppause, SCFL_REPLAY)
{
	BhopPlayer *player = g_pBhopPlayerManager->ToPlayer(controller);
	if (!player)
	{
	...
	Bhop::replaysystem::commands::ToggleReplayPause(player);
	return MRES_SUPERCEDE;
}

SCMD(bhop_rploadprogress, SCFL_REPLAY)
{
	BhopPlayer *player = g_pBhopPlayerManager->ToPlayer(controller);
	if (!player)
	{
	...
	Bhop::replaysystem::commands::CheckReplayLoadProgress(player);
	return MRES_SUPERCEDE;
}

SCMD(bhop_rpcancelload, SCFL_REPLAY)
{
	BhopPlayer *player = g_pBhopPlayerManager->ToPlayer(controller);
	if (!player)
	{
	...
	Bhop::replaysystem::commands::CancelReplayLoad(player);
	return MRES_SUPERCEDE;
}

SCMD(bhop_replays, SCFL_REPLAY)
{
	BhopPlayer *player = g_pBhopPlayerManager->ToPlayer(controller);
	if (!player)
	{
	...
	Bhop::replaysystem::commands::ListReplays(player, args->ArgS());
	return MRES_SUPERCEDE;
}

SCMD(bhop_rphidelegs, SCFL_REPLAY)
{
	BhopPlayer *player = g_pBhopPlayerManager->ToPlayer(controller);
	if (!player)
	{
	...
	Bhop::replaysystem::commands::ToggleLegsVisibility(player);
	return MRES_SUPERCEDE;
}
