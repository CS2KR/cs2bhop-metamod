...
CConVar<bool> bhop_replay_playback_debug("bhop_replay_playback_debug", FCVAR_NONE, "Prints debug info about replay playback.", false);
CConVar<bool> bhop_replay_playback_skins_enable("bhop_replay_playback_skins_enable", FCVAR_NONE,
												"Enables applying player skins during replay playback.", true);

static Bhop::replaysystem::data::ReplayPlayback g_currentReplay = {};
static Bhop::replaysystem::data::AsyncLoadStatus g_loadStatus = {};
static std::thread g_loadThread;
static std::atomic<bool> g_cancelLoad {false};

namespace Bhop::replaysystem::data
{
	...
	f32 GetReplayTime()
	{
		return g_currentReplay.startTime == 0.0f ? 0.0f : (g_pBhopUtils->GetServerGlobals()->curtime - g_currentReplay.startTime);
	}
	...
	static_function void UpdateProgress(const char *cursor, const char *dataStart, size_t totalSize, std::atomic<f32> &progress)
	{
		if (totalSize > 0)
		{
			...
			if (bhop_replay_playback_debug.Get())
			{
														...
														std::atomic<bool> &shouldCancel)
	{
		...
		if (result.header.version() < 1 || result.header.version() > BHOP_REPLAY_VERSION)
		{
			return result;
		}
		...
		if (!Bhop::replaysystem::compression::ReadTickDataCompressed(cursor, end, tickDataVec, subtickDataVec, result.header.version()))
		{
			return result;
		}
		...
		if (!Bhop::replaysystem::compression::ReadWeaponsCompressed(cursor, end, weaponTableVec))
		{
			delete[] result.tickData;
			delete[] result.subtickData;
			return {};
		...
		if (!Bhop::replaysystem::compression::ReadEventsCompressed(cursor, end, eventsVec))
		{
			delete[] result.tickData;
			delete[] result.subtickData;
			delete[] result.weaponIndices;
