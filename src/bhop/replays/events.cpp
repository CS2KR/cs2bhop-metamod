...
namespace Bhop::replaysystem::events
{
	...
	void HandleTimerEvent(BhopPlayer &player, const RpEvent *event, data::ReplayPlayback *replay)
	{
		...
		switch (event->data.timer.type)
		{
			...
			{
				const BhopCourseDescriptor *course = Bhop::course::GetCourseByCourseID(event->data.timer.index);
				...
				replay->startTime = g_pBhopUtils->GetServerGlobals()->curtime - event->data.timer.time;
				replay->endTime = 0.0f;
			...
			{
				const BhopCourseDescriptor *courseDesc = Bhop::course::GetCourseByCourseID(event->data.timer.index);
				...
				{
					BhopStyleService *style = player.styleServices[i];
	...
	void HandleModeChangeEvent(BhopPlayer &player, const RpEvent *event)
	{
		...
		g_pBhopModeManager->SwitchToMode(&player, modeNamePtr, true, true, false);
	}
	...
	void HandleStyleChangeEvent(BhopPlayer &player, const RpEvent *event)
	{
		...
		{
			g_pBhopStyleManager->ClearStyles(&player, true, false);
		}
		if (styleNamePtr[0] != '\0')
		{
			g_pBhopStyleManager->AddStyle(&player, styleNamePtr, true, false);
		}
	}
	...
	void ReprocessEventsUpToTick(data::ReplayPlayback *replay, u32 targetTick)
	{
		...
		else
		{
			...
			g_pBhopModeManager->SwitchToMode(player, "128tick", true, false, false);
			g_pBhopStyleManager->ClearStyles(player, true, false);
		...
		for (u32 i = startEventIndex; i < replay->numEvents; i++)
		{
			...
			switch (event->type)
			{
				...
				case RPEVENT_MODE_CHANGE:
				{
					g_pBhopModeManager->SwitchToMode(player, event->data.modeChange.name, true, false, false);
					break;
				}
				case RPEVENT_STYLE_CHANGE:
				{
					...
					{
						g_pBhopStyleManager->AddStyle(player, event->data.styleChange.name, true, false);
					}
					break;
				}
				case RPEVENT_TELEPORT:
