...
static const char *ARCHIVE_INDEX_PATH = BHOP_REPLAY_PATH "/archive_index.txt";
...
void ReplayWatcher::FilterAndPrintMatchingCheaterReplays(ReplayFilterCriteria &criteria, BhopPlayer *player)
{
	...
	CUtlString headers[BHOP_ARRAYSIZE(cheaterReplayTableHeaders)];
	for (u32 i = 0; i < BHOP_ARRAYSIZE(cheaterReplayTableHeaders); i++)
	{
		headers[i] = player->languageService->PrepareMessage(cheaterReplayTableHeaders[i]).c_str();
	}

	utils::Table<BHOP_ARRAYSIZE(cheaterReplayTableHeaders)> table(player->languageService->PrepareMessage(CHEATER_REPLAY_TABLE_KEY).c_str(), headers);
...
void ReplayWatcher::FilterAndPrintMatchingRunReplays(ReplayFilterCriteria &criteria, BhopPlayer *player)
{
	std::vector<std::pair<UUID_t, ReplayHeader>> matchingReplays;
	u64 numMatched = 0;
	{
	...
	CUtlString headers[BHOP_ARRAYSIZE(runReplayTableHeaders)];
	for (u32 i = 0; i < BHOP_ARRAYSIZE(runReplayTableHeaders); i++)
	{
		headers[i] = player->languageService->PrepareMessage(runReplayTableHeaders[i]).c_str();
	}
	utils::Table<BHOP_ARRAYSIZE(runReplayTableHeaders)> table(player->languageService->PrepareMessage(RUN_REPLAY_TABLE_KEY).c_str(), headers);
	for (size_t i = 0; i < matchingReplays.size(); i++)
	{
		auto &pair = matchingReplays[i];
		const ReplayHeader &hdr = pair.second;
...
void ReplayWatcher::FilterAndPrintMatchingManualReplays(ReplayFilterCriteria &criteria, BhopPlayer *player)
{
	std::vector<std::pair<UUID_t, ReplayHeader>> matchingReplays;
	u64 numMatched = 0;
	{
	...
	CUtlString headers[BHOP_ARRAYSIZE(manualReplayTableHeaders)];
	for (u32 i = 0; i < BHOP_ARRAYSIZE(manualReplayTableHeaders); i++)
	{
		headers[i] = player->languageService->PrepareMessage(manualReplayTableHeaders[i]).c_str();
	}
	utils::Table<BHOP_ARRAYSIZE(manualReplayTableHeaders)> table(player->languageService->PrepareMessage(MANUAL_REPLAY_TABLE_KEY).c_str(), headers);
	for (size_t i = 0; i < matchingReplays.size(); i++)
	{
		auto &pair = matchingReplays[i];
		const ReplayHeader &hdr = pair.second;
...
void ReplayWatcher::PrintUsage(BhopPlayer *player)
{
	if (!player)
	{
		return;
...
void ReplayWatcher::FindReplaysMatchingCriteria(const char *inputs, BhopPlayer *player)
{
	// Parse inputs and filter replays accordingly.
	KeyValues3 params;
	utils::ParseArgsToKV3(inputs, params, const_cast<const char **>(generalParameters), BHOP_ARRAYSIZE(generalParameters));
	...
	criteria.mapName = kv ? kv->GetString() : g_pBhopUtils->GetCurrentMapName().Get();
	...
	switch (criteria.type)
	{
		case RP_CHEATER:
		{
			utils::ParseArgsToKV3(inputs, params, const_cast<const char **>(cheaterParameters), BHOP_ARRAYSIZE(cheaterParameters));
		...
		case RP_RUN:
		{
			utils::ParseArgsToKV3(inputs, params, const_cast<const char **>(runParameters), BHOP_ARRAYSIZE(runParameters));
		...
		case RP_MANUAL:
		{
			utils::ParseArgsToKV3(inputs, params, const_cast<const char **>(manualParameters), BHOP_ARRAYSIZE(manualParameters));
									  ...
									  u64 currentTime)
{
	...
	int maxPer = MAX(BhopOptionService::GetOptionInt("maxRunReplaysPerGroup", 3), 2);
...
{
	int maxManual = MAX(BhopOptionService::GetOptionInt("maxManualReplays", 2), 2);
	for (auto &[steamID, vec] : bySteam)
	{
		if (vec.size() > maxManual)
		{
			...
			for (size_t i = maxManual; i < vec.size(); i++)
			{
				...
				V_snprintf(fullPath, sizeof(fullPath), "%s/%s.replay", BHOP_REPLAY_PATH, vec[i].first.ToString().c_str());
...
void ReplayWatcher::ScanReplays()
{
	...
	V_snprintf(searchPath, sizeof(searchPath), "%s/*.replay", BHOP_REPLAY_PATH);
	...
	u32 retentionMinutes = MAX(BhopOptionService::GetOptionInt("archiveRetentionMinutes", 2880), 1440);
	...
	while (pFileName)
	{
		if (!g_pFullFileSystem->FindIsDirectory(findHandle))
		{
			...
			{
				char fullPath[MAX_PATH];
				V_snprintf(fullPath, sizeof(fullPath), "%s/%s", BHOP_REPLAY_PATH, pFileName);
...
void Bhop::replaysystem::InitWatcher()
{
	g_ReplayWatcher.Start();
}

void Bhop::replaysystem::CleanupWatcher()
{
