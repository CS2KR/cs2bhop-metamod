...
void Bhop::replaysystem::item::InitItemAttributes()
{
	KeyValues *kv = new KeyValues("items_game");
	kv->LoadFromFile(g_pFullFileSystem, "scripts/items/items_game.txt", "GAME");
	if (!kv)
	{
		Warning("[Bhop] Failed to load items_game.txt for item attributes!\n");
		delete kv;
		return;
	}
	...
	if (!subKey)
	{
		Warning("[Bhop] Failed to find attributes key in items_game.txt!\n");
		delete kv;
		return;
	}
	...
	if (!subKey)
	{
		Warning("[Bhop] Failed to find paint_kits key in items_game.txt!\n");
		delete kv;
		return;
	}
	for (KeyValues *sub = subKey->GetFirstSubKey(); sub != NULL; sub = sub->GetNextKey())
	...
	if (!subKey)
	{
		Warning("[Bhop] Failed to find items key in items_game.txt!\n");
		delete kv;
		return;
	}
	for (KeyValues *sub = subKey->GetFirstSubKey(); sub != NULL; sub = sub->GetNextKey())
...
std::string Bhop::replaysystem::item::GetItemAttributeName(u16 id)
{
	return itemAttributes[id];
}

std::string Bhop::replaysystem::item::GetWeaponName(u16 id)
{
...
gear_slot_t Bhop::replaysystem::item::GetWeaponGearSlot(u16 id)
{
	return weaponItemMapping[id].gearSlot;
}

bool Bhop::replaysystem::item::DoesPaintKitUseLegacyModel(float paintKit)
{
	return paintKitUsesLegacyModel[static_cast<u32>(paintKit)];
}

void Bhop::replaysystem::item::ApplyItemAttributesToWeapon(CBasePlayerWeapon &weapon, const EconInfo &info)
{
	...
	bool isKnife = false;
	if (BHOP_STREQI(weapon.GetClassname(), "weapon_knife") || BHOP_STREQI(weapon.GetClassname(), "weapon_knife_t"))
	{
	...
	for (int i = 0; i < info.mainInfo.numAttributes; i++)
	{
		...
		if (id == 6)
		{
			...
			bool usesLegacyModel = Bhop::replaysystem::item::DoesPaintKitUseLegacyModel(value);
			if (!isKnife)
			{
		...
		g_pBhopUtils->SetOrAddAttributeValueByName(&item.m_AttributeList(), Bhop::replaysystem::item::GetItemAttributeName(id).c_str(), value);
		g_pBhopUtils->SetOrAddAttributeValueByName(&attributeList, Bhop::replaysystem::item::GetItemAttributeName(id).c_str(), value);
	}
}

void Bhop::replaysystem::item::ApplyModelAttributesToPawn(CCSPlayerPawn *pawn, const EconInfo &info, const char *modelName)
{
	...
	{
		g_pBhopUtils->SetModel(pawn, modelName);
	}
	else
	{
		META_CONPRINTF("[Bhop] Model '%s' does not exist, skipping model application.\n", modelName);
	}
