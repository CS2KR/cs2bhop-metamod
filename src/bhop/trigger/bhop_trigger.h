...
class BhopTriggerService : public BhopBaseService
{
public:
	...
	struct TriggerTouchTracker
	{
		...
		const BhopTrigger *bhopTrigger = NULL;
		...
		bool CanEndTouch()
		{
			...
			if (!startedTouch)
			{
				META_CONPRINTF("[Bhop::Trigger] EndTouch check is called before the start touch interaction even finished!\n");
				return false;
			}
			return true;
		}
...
private:
	...
	void AddPushEvent(const BhopTrigger *trigger);
...
inline bool operator==(const BhopTriggerService::TriggerTouchTracker &left, const BhopTriggerService::TriggerTouchTracker &right)
{
	return left.triggerHandle == right.triggerHandle;
}

inline bool operator==(const BhopTriggerService::PushEvent &left, const BhopTriggerService::PushEvent &right)
{
	assert(left.source);
	assert(right.source);
