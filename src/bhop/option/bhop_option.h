...
class BhopOptionServiceEventListener
{
public:
	virtual void OnPlayerPreferencesLoaded(BhopPlayer *player) {};
	virtual void OnPlayerPreferenceChanged(BhopPlayer *player, const char *optionName) {};
};

class BhopOptionService : public BhopBaseService
{
	using BhopBaseService::BhopBaseService;

	DECLARE_CLASS_EVENT_LISTENER(BhopOptionServiceEventListener);
