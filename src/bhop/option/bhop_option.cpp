#include "bhop_option.h"
#include "bhop/db/bhop_db.h"
#include "utils/eventlisteners.h"
static_global KeyValues *pServerCfgKeyValues;

IMPLEMENT_CLASS_EVENT_LISTENER(BhopOptionService, BhopOptionServiceEventListener);
...
void BhopOptionService::LoadDefaultOptions()
{
	char serverCfgPath[1024];
	V_snprintf(serverCfgPath, sizeof(serverCfgPath), "%s%s", g_SMAPI->GetBaseDir(), "/cfg/cs2bhop-server-config.txt");

	pServerCfgKeyValues = new KeyValues("ServerConfig");
	pServerCfgKeyValues->LoadFromFile(g_pFullFileSystem, serverCfgPath, nullptr);
}

const char *BhopOptionService::GetOptionStr(const char *optionName, const char *defaultValue)
{
	return pServerCfgKeyValues->GetString(optionName, defaultValue);
}

f64 BhopOptionService::GetOptionFloat(const char *optionName, f64 defaultValue)
{
	return pServerCfgKeyValues->GetFloat(optionName, defaultValue);
}

i64 BhopOptionService::GetOptionInt(const char *optionName, i64 defaultValue)
{
	return pServerCfgKeyValues->GetInt(optionName, defaultValue);
}

KeyValues *BhopOptionService::GetOptionKV(const char *optionName)
{
	return pServerCfgKeyValues->FindKey(optionName);
}

void BhopOptionService::InitOptions()
{
	LoadDefaultOptions();
}

void BhopOptionService::Cleanup()
{
...
void BhopOptionService::InitializeLocalPrefs(CUtlString text)
{
	...
	if (!error.IsEmpty())
	{
		META_CONPRINTF("[Bhop::DB] Error fetching local preference: %s\n", error.Get());
		return;
	}
...
void BhopOptionService::InitializeGlobalPrefs(std::string json)
{
	...
	if (!error.IsEmpty())
	{
		META_CONPRINTF("[Bhop::Options] Error loading global preferences: %s\n", error.Get());
		return;
	}
	...
	META_CONPRINTF("[Bhop::Options] Loaded global preferences.\n");
...
void BhopOptionService::SaveLocalPrefs()
{
	...
	if (!error.IsEmpty())
	{
		META_CONPRINTF("[Bhop::DB] Error saving local preference: %s\n", error.Get());
		return;
	}
	this->player->databaseService->SavePrefs(output);
}

void BhopOptionService::OnPlayerActive()
{
