...
struct Recorder
{
	...
	Recorder(BhopPlayer *player, f32 numSeconds, ReplayType type, bool copyTimerEvents);
	~Recorder();
	...
	static void Init(ReplayHeader &hdr, BhopPlayer *player, ReplayType type);
};
...
{
	RunRecorder(BhopPlayer *player);
	void End(f32 time, i32 numTeleports);
};

struct CheaterRecorder : public Recorder
{
	CheaterRecorder(BhopPlayer *player, const char *reason, BhopPlayer *savedBy);
};

struct ManualRecorder : public Recorder
{
	ManualRecorder(BhopPlayer *player, f32 duration, BhopPlayer *savedBy);
};

class BhopRecordingService : public BhopBaseService
{
	using BhopBaseService::BhopBaseService;

public:
	~BhopRecordingService()
	{
...
public:
	...
	void WriteCircularBufferToFileAsync(f32 duration, const char *cheaterReason, BhopPlayer *saver, DiskWriteSuccessCallback onSuccess,
...
public:
	...
	BhopModeManager::ModePluginInfo lastKnownMode;
	std::vector<BhopStyleManager::StylePluginInfo> lastKnownStyles;
