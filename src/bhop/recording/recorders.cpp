...
#include "bhop/replays/compression.h"

extern CConVar<bool> bhop_replay_recording_debug;

ManualRecorder::ManualRecorder(BhopPlayer *player, f32 duration, BhopPlayer *savedBy) : Recorder(player, duration, RP_MANUAL, true)
{
	if (savedBy)
	{
...
CheaterRecorder::CheaterRecorder(BhopPlayer *player, const char *reason, BhopPlayer *savedBy) : Recorder(player, 120.0f, RP_CHEATER, true)
{
...
RunRecorder::RunRecorder(BhopPlayer *player) : Recorder(player, 5.0f, RP_RUN, true)
{
	auto *runProto = replayHeader.mutable_run();
	runProto->set_course_name(player->timerService->GetCourse()->GetName().Get());
	auto modeInfo = Bhop::mode::GetModeInfo(player->modeService);
...
void RunRecorder::End(f32 time, i32 numTeleports)
{
	...
	this->desiredStopTime = g_pBhopUtils->GetServerGlobals()->curtime + 4.0f;
}

// Recorder Implementation
Recorder::Recorder(BhopPlayer *player, f32 numSeconds, ReplayType type, bool copyTimerEvents)
{
...
void Recorder::FlushChunkToDisk()
{
	if (tempFileBase.empty())
	{
		...
		char path[512];
		V_snprintf(path, sizeof(path), "%s/.tmp_%s", BHOP_REPLAY_PATH, uuidStr.c_str());
		tempFileBase = path;
	}
...
bool Recorder::WriteToFile()
{
	...
	V_snprintf(finalFilename, sizeof(finalFilename), "%s/%s.replay", BHOP_REPLAY_PATH, uuidStr.c_str());
	...
	{
		META_CONPRINTF("bhop_replay_recording_debug: Saved replay to %s (%zu bytes)\n", finalFilename, buffer.size());
	}
...
i32 Recorder::WriteHeader(std::vector<char> &outBuffer)
{
	...
	{
		META_CONPRINTF("[Bhop] Failed to serialize replay header protobuf\n");
		return 0;
	}
...
bool Recorder::WriteToMemory(std::vector<char> &outBuffer)
{
	...
	Bhop::replaysystem::compression::WriteTickDataCompressed(outBuffer, allTickData, unpackedSubtick);
	Bhop::replaysystem::compression::WriteWeaponsCompressed(outBuffer, this->weaponTable);
	Bhop::replaysystem::compression::WriteEventsCompressed(outBuffer, allEvents);
	Bhop::replaysystem::compression::WriteCmdDataCompressed(outBuffer, allCmdData, unpackedCmdSubtick);
