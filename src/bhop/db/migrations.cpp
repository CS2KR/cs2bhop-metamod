...
void BhopDatabaseService::RunMigrations()
{
	Transaction txn;
	...
	GetDatabaseConnection()->ExecuteTransaction(txn, BhopDatabaseService::CheckMigrations, OnGenericTxnFailure);
}

void BhopDatabaseService::CheckMigrations(std::vector<ISQLQuery *> queries)
{
	...
	if (current > max)
	{
		META_CONPRINTF("[Bhop::DB] Fatal error: Number of current migrations are higher than the maximum!\n");
		return;
	}
	...
	for (u32 i = 0; i < current; i++)
	{
		...
		if (currentCRC != crc)
		{
			META_CONPRINTF("[Bhop::DB] Fatal error: Migration query %s with CRC %lu does not match the database's %lu!\n", migrationQuery.c_str(),
						   crc, currentCRC);
			META_CONPRINT("[Bhop::DB] Database migration failed. LocalDB will not be available.");
	...
	auto onSuccess = []()
	{
		META_CONPRINT("[Bhop::DB] Database migration successful.\n");
...
bool BhopDatabaseService::IsReady()
{
