...
void BhopDatabaseService::SetupDatabase()
{
	KeyValues *config = BhopOptionService::GetOptionKV("db");
	if (!config)
	{
		META_CONPRINT("[Bhop::DB] No database config detected.\n");
		return;
	}
	ISQLInterface *sqlInterface = (ISQLInterface *)g_SMAPI->MetaFactory(SQLMM_INTERFACE, nullptr, nullptr);
	if (!sqlInterface)
	{
		META_CONPRINT("[Bhop::DB] Database plugin not found. Local database is disabled.\n");
		return;
	}
	...
	if (!V_stricmp(driver, "sqlite"))
	{
		...
		char path[MAX_PATH];
		V_snprintf(path, sizeof(path), "addons/cs2bhop/data/%s.sqlite3", config->GetString("database"));
		info.database = path;
...
void BhopDatabaseService::OnDatabaseConnected(bool connect)
{
	if (connect)
	{
		META_CONPRINT("[Bhop::DB] LocalDB connected.\n");
		BhopDatabaseService::RunMigrations();
	}
	else
	{
		META_CONPRINT("[Bhop::DB] Failed to connect\n");
