...
BhopSidewaysStylePlugin g_BhopSidewaysStylePlugin;

CGameConfig *g_pGameConfig = NULL;
BhopUtils *g_pBhopUtils = NULL;
BhopStyleManager *g_pStyleManager = NULL;
StyleServiceFactory g_StyleFactory = [](BhopPlayer *player) -> BhopStyleService * { return new BhopSidewaysStyleService(player); };
PLUGIN_EXPOSE(BhopSidewaysStylePlugin, g_BhopSidewaysStylePlugin);

const char *incompatibleStyles[] = {"HSW", "OnlyW"};

bool BhopSidewaysStylePlugin::Load(PluginId id, ISmmAPI *ismm, char *error, size_t maxlen, bool late)
{
	PLUGIN_SAVEVARS();
	// Load mode
	int success;
...
bool BhopSidewaysStylePlugin::Unload(char *error, size_t maxlen)
{
	g_pStyleManager->UnregisterStyle(g_PLID);
	return true;
}

bool BhopSidewaysStylePlugin::Pause(char *error, size_t maxlen)
{
	g_pStyleManager->UnregisterStyle(g_PLID);
	return true;
}

bool BhopSidewaysStylePlugin::Unpause(char *error, size_t maxlen)
{
...
void BhopSidewaysStyleService::Init() {}

const CVValue_t *BhopSidewaysStyleService::GetTweakedConvarValue(const char *name)
{
	return nullptr;
}

void BhopSidewaysStyleService::Cleanup() {}

void BhopSidewaysStyleService::OnSetupMove(PlayerCommand *pc)
{
