...
#define BHOP_STYLE_MANAGER_INTERFACE "BhopStyleManagerInterface"

// TODO styles: normal, backwards, sw, hsw, w only, lowgrav, autobhop, 250 speed, high gravity, notrigger, alivestrafe
class BhopStyleService : public BhopBaseService
{
...
typedef BhopStyleService *(*StyleServiceFactory)(BhopPlayer *player);

class BhopStyleManager
{
public:
	...
	void AddStyle(BhopPlayer *player, const char *styleName, bool silent = false, bool updatePreference = true);
	void RemoveStyle(BhopPlayer *player, const char *styleName, bool silent = false, bool updatePreference = true);
	void ToggleStyle(BhopPlayer *player, const char *styleName, bool silent = false, bool updatePreference = true);
	void ClearStyles(BhopPlayer *player, bool silent = false, bool updatePreference = true);
	void RefreshStyles(BhopPlayer *player, bool updatePreference = true);
	CUtlString GetStylesString(BhopPlayer *player);
	void PrintActiveStyles(BhopPlayer *player);
	void PrintAllStyles(BhopPlayer *player);

private:
};

extern BhopStyleManager *g_pBhopStyleManager;

namespace Bhop::style
{
	void InitStyleManager();
	void LoadStylePlugins();
	void UpdateStyleDatabaseID(CUtlString name, i32 id);
	BhopStyleManager::StylePluginInfo GetStyleInfo(BhopStyleService *style);
	BhopStyleManager::StylePluginInfo GetStyleInfo(CUtlString styleName);
